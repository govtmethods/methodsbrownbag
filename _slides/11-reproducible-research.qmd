...
<!--
FLAG: skeleton, not finished slides.

The deck now follows the 17-point outline agreed for the session; each slide is
marked with its outline number in a comment above the heading so the two stay
traceable. Everything below a heading is a PLACEHOLDER — the wording, the
examples and the exercises are to be written in a later pass. Do not read the
prose here as drafted content.

Two things to know before editing:

1. The absolute/relative paths figure (outline 6) is a COPY of the
   `fig-repro-paths` chunk in chapters/11-reproducible-research.qmd, not a
   shared file. If the figure changes, it has to change in both places.
2. Items the outline itself left open are marked `[?]` in the outline and
   carried here as `FLAG: OPEN` comments. Resolve them with the presenter
   rather than inventing an answer.
-->

**Across time** — the you of next November re-runs what the you of this
September wrote.

**Across space** — it runs on a machine that is not yours.

<!-- FLAG: placeholder. Decide whether the title slide's YAML is enough and
     this framing slide moves to the motivation block instead. -->

<!-- 2 — Motivation -->
# Data and code live in weird places
...
<!-- FLAG: placeholder. A short before/after of an uncommented vs. commented
     script. Keep it to one screenful. -->

<!-- 5 — Roadmap -->
# Part 1 — File Management and Workflow

## The first half {.smaller}

This comes *on top of* well-commented code and judiciously named objects.
...
## Standardized folder structure — Expanded {.smaller}
...
<!-- Copied from the chapter's "Standardized Folder and File Structure" tree.
     One deliberate difference: the chapter closes 003-manuscript.qmd with `└─`
     in the middle of the list, which draws a broken tree on a projector. It is
     `├─` here. Worth fixing in the chapter too. -->

## Exercise — build the minimal one by hand {.smaller}

Point and click. That is the whole exercise.

::: {.incremental}
1. Make a folder called `govt-8001-dataessay`, wherever your coursework lives
2. Inside it, make three more: `Data`, `Scripts`, `Outputs`
3. In RStudio: `File > New Project > Existing Directory`, and point it at that folder
:::

. . .

**Checkpoint** — `govt-8001-dataessay.Rproj` now sits in the folder, and RStudio's
top-right corner names the project.

## …or let a script do it {.smaller}
...
::: {.fragment}
It creates `Data/`, `Scripts/`, `Outputs/` and every subfolder, plus a starter
`.gitignore`. Running it twice is safe: it leaves anything that already exists
alone. Windows, macOS or Linux — it builds every path with `here()`, so there are
no backslash problems.
:::

<!-- FLAG: this pair now covers what outline 10 ("Let's work — manual folder
     creation / link setup / start R project / run setup") was going to do.
     Decide whether that later slide becomes a recap or goes. -->

<!-- 11 — Version control -->
# Part 2 — Version Control with Git

## The second half {.smaller}
...
![](../chapters/git-vcs.png){.nostretch width="88%" fig-align="center"}
...
:::
::: {.column width="55%"}
![](../chapters/git-stages.png){.nostretch width="100%"}
