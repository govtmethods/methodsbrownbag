...
# TODO: add `date: "Month D, YYYY"` once the session is scheduled, then check
# that 11 is still the right number in _quarto.yml (chapter 10 is Sept 2026).
---
...
The series has covered the first two topics before, in more depth than one
session allows:

- [File Management and Workflow](01-intro.html) (August 2025) — absolute versus
  relative paths, R Projects, the `here` package, a standard folder layout
- [Version Control with Git and Github](02-github.html) (September 2025) —
  installation, the staging model, remotes and GitHub

Read those as the reference. This chapter revisits the same ground as a single
workflow, and ends somewhere they do not.
:::
